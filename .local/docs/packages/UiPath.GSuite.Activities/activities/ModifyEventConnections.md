...
| `ChangeAttachments` | Change Attachments | `Property` | [`EventPropertyChange`](#enum-reference) | No | `NoChange` | How to modify the attachments list. |
| `OverwriteAttachments` | Overwrite Attachments | `InArgument` | `IEnumerable<`[`GDriveRemoteItem`](types/GDriveRemoteItem.md)`>` | No | | Drive files that replace the existing attachments (Overwrite mode). |
| `AddAttachments` | Add Attachments | `InArgument` | `IEnumerable<`[`GDriveRemoteItem`](types/GDriveRemoteItem.md)`>` | No | | Drive files to add to the attachments (AddRemove mode). |
| `RemoveAttachments` | Remove Attachments | `InArgument` | `IEnumerable<`[`GDriveRemoteItem`](types/GDriveRemoteItem.md)`>` | No | | Drive files to remove from the attachments (AddRemove mode). |
...
- Attachments follow the same tri-state pattern as attendees: `ChangeAttachments = NoChange` leaves them untouched (attachment inputs are ignored), `Overwrite` replaces the whole list (an empty/unset overwrite input clears all attachments), `AddRemove` merges additions and removals into the event's current attachments. Removals are matched by Drive file id.
- Attachments are references to Google Drive **files** — folders are rejected with a business error. Attachment inputs take collections of [`GDriveRemoteItem`](types/GDriveRemoteItem.md) values, e.g. the output of [`UploadFilesConnections`](UploadFilesConnections.md) or [`GetFileFolderConnections`](GetFileFolderConnections.md).
- The modified event's attachments are returned in the output's `Attachments` collection ([`GSuiteEventAttachment`](types/GSuiteEventAttachment.md)).
