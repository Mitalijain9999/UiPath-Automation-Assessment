...
| `Attachments` | Attachments | `InArgument` | `IEnumerable<`[`GDriveRemoteItem`](types/GDriveRemoteItem.md)`>` | No | | The Google Drive files to attach to the event. Only files can be attached, not folders. |
...
- Attachments are references to Google Drive **files** — folders are rejected with a business error. Attaching does not grant attendees access to the file; share it separately (e.g. via [`ShareFileFolderConnections`](ShareFileFolderConnections.md)).
- `Attachments` takes a collection of [`GDriveRemoteItem`](types/GDriveRemoteItem.md) values, e.g. the output of [`UploadFilesConnections`](UploadFilesConnections.md) or [`GetFileFolderConnections`](GetFileFolderConnections.md). If only a Drive URL is available, resolve it to an item first with [`GetFileFolderConnections`](GetFileFolderConnections.md) (its `UrlOrId` mode accepts full URLs).
- The created event's attachments are returned in the output's `Attachments` collection ([`GSuiteEventAttachment`](types/GSuiteEventAttachment.md)).
