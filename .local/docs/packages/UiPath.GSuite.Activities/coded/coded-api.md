...
- `GSuiteEventItem Item` — The underlying event model (includes attendees, start/end times, attachments, etc.). `Item.Attachments` exposes the Drive files attached to the event as `GSuiteEventAttachment` (`Title`, `FileUrl`, `MimeType`, `IconLink`, `FileId`).
- `string Id` — Event ID.
...
- `.WithAttachments(params IFile[] files)` / `.WithAttachments(IEnumerable<IFile> files)` — Attach Google Drive files to the event.

**Attachment notes:** Google Calendar attachments are references to Google Drive **files** (folders cannot be attached). Duplicate files (same Drive file id) are attached only once. Attaching does not grant attendees access to the file; share it separately (e.g. `file.Share(...)`). To attach a spreadsheet or document handle (`ISpreadsheet`/`IDocument` are not `IFile`), convert its `Item` first: `spreadsheet.Item.ToFile(driveService)`. The event's attachments are read back via `ICalendarEvent.Item.Attachments` (`GSuiteEventAttachment`: `Title`, `FileUrl`, `MimeType`, `IconLink`, `FileId` — only `FileUrl`/`Title`/`MimeType` are writable at Google; `IconLink`/`FileId` are populated by Google).

---

### `UpdateCalendarItem`

Fluent builder for modifying an existing calendar event. Initialized with the current event's attendees and attachments.
...
- `.ClearAttachments()` / `.OverwriteAttachments(...)` / `.AddAttachments(...)` / `.RemoveAttachments(...)` — Modify the event's Drive file attachments. Each accepts `params IFile[]` or `IEnumerable<IFile>`. Removals are matched by Drive file id. If no attachment method is called, the event's attachments are left untouched. The attachment notes on `CreateCalendarItem` apply here too.
